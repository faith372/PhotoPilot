...
当前版本是浏览器 + 本地 Node 服务原型：默认不上传照片，先使用本地 Canvas 像素指标评分；用户授权后，可通过本机服务代理调用主流大模型进行复核评分和修图方案生成。
...
- 支持自然语言生成调色方案：默认使用本地关键词规则，云端授权后可调用大模型生成结构化修图参数。
...
PhotoPilot 默认不上传照片。选择云端复核评分或云端修图方案后，前端会把压缩缩略图、本地评分指标、当前编辑参数和用户提示词发送到本机 Node 服务，再由本机服务调用模型 API。
...
| `app.js` | 照片状态、本地像素评分、AI 复核、AI 修图方案、编辑、批量同步和导出逻辑 |
| `server.mjs` | 本地静态预览服务、大模型评分代理和修图方案代理 |
...
2. 增加项目保存和重新打开能力。
3. 增加批量导出 ZIP 或桌面目录选择，减少浏览器逐张下载限制。
4. 评估 Electron 或 Tauri 桌面端封装。
5. 增加人像增强与 RAW 工作流。
